...
from platemap.lib.sql_connection import Transaction, TRN
from platemap.lib.config_manager import pm_config


DB_TEST_TABLE = """CREATE TABLE barcodes.test_table (
...
class TestBase(TestCase):
    def setUp(self):
        ...
        with connect(user=pm_config.user, password=pm_config.password,
                     host=pm_config.host, port=pm_config.port,
                     database=pm_config.database) as con:
    ...
    def tearDown(self):
        ...
        with connect(user=pm_config.user, password=pm_config.password,
                     ...
                     database=pm_config.database) as con:
            with con.cursor() as cur:
                cur.execute('DROP TABLE barcodes.test_table')

    def _populate_test_table(self):
        """Aux function that populates the test table"""
        sql = """INSERT INTO barcodes.test_table
    ...
    def _assert_sql_equal(self, exp):
        ...
        with connect(user=pm_config.user, password=pm_config.password,
                     ...
                     database=pm_config.database) as con:
            with con.cursor() as cur:
                cur.execute("SELECT * FROM barcodes.test_table")
                obs = cur.fetchall()
            con.commit()
...
class TestTransaction(TestBase):
    ...
    def test_add(self):
        with TRN:
            ...
            sql1 = "INSERT INTO barcodes.test_table (bool_column) VALUES (%s)"
            args1 = [True]
            TRN.add(sql1, args1)
            sql2 = "INSERT INTO barcodes.test_table (int_column) VALUES (1)"
            TRN.add(sql2)
            args3 = (False,)
            TRN.add(sql1, args3)
            sql3 = "INSERT INTO barcodes.test_table (int_column) VALEUS (%(foo)s)"
            args4 = {'foo': 1}
            TRN.add(sql3, args4)
